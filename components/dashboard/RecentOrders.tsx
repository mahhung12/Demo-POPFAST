import { Table, TableBody, TableCell, TableHeader, TableRow } from "../ui/table";
import { Tooltip, TooltipContent, TooltipTrigger, TooltipProvider } from "../ui/tooltip";

export default function RecentOrders({ events }: { events: any }) {
  // Transform the events data into a format suitable for the table
  const tableData = events.pageviews.map((event) => ({
    id: event.id,
    name: event.url || "Unknown URL", // Use the URL as the product name
    category: event.referrer || "Unknown Referrer", // Use the referrer as the category
    country: event.country || "Unknown Country", // Use the country field
    timestamp: event.timestamp || "Unknown Timestamp",
    browser: event.browser || "Unknown Browser", // Use the browser field
  }));

  // Define the columns to render
  const columns = [
    { key: "name", label: "URL", formatter: (value: string) => formatUrl(value) },
    { key: "timestamp", label: "Date", formatter: (value: string) => new Date(value).toLocaleString() },
    // { key: "category", label: "Referrer", formatter: (value: string) => formatUrl(value) },
    { key: "country", label: "Country" },
    { key: "browser", label: "Browser" },
  ];

  const formatUrl = (url: string) => {
    try {
      const parsedUrl = new URL(url);
      return parsedUrl.hostname + parsedUrl.pathname;
    } catch {
      return url || "Invalid URL";
    }
  };

  return (
    <TooltipProvider>
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
        <div className="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Recent Activities</h3>
          <div className="flex items-center gap-3">
            <button className="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
              Filter
            </button>
            <button className="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
              See all
            </button>
          </div>
        </div>
        <div className="max-w-full overflow-x-auto">
          <Table>
            {/* Table Header */}
            <TableHeader className="border-gray-100 dark:border-gray-800 border-y">
              <TableRow>
                {columns.map((column) => (
                  <TableCell
                    key={column.key}
                    isHeader
                    className="py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"
                  >
                    {column.label}
                  </TableCell>
                ))}
              </TableRow>
            </TableHeader>

            {/* Table Body */}
            <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
              {tableData.map((product) => (
                <TableRow key={product.id}>
                  {columns.map((column) => (
                    <TableCell
                      key={column.key}
                      className="py-3 px-3 text-start whitespace-nowrap truncate text-gray-500 text-theme-sm dark:text-gray-400"
                    >
                      <Tooltip>
                        <TooltipTrigger>
                          <p className="font-medium text-gray-800 text-theme-sm dark:text-white/90">
                            {column.formatter ? column.formatter(product[column.key]) : product[column.key]}
                          </p>
                        </TooltipTrigger>
                        <TooltipContent>
                          <p>{product[column.key]}</p>
                        </TooltipContent>
                      </Tooltip>
                    </TableCell>
                  ))}
                </TableRow>
              ))}
            </TableBody>
          </Table>
        </div>
      </div>
    </TooltipProvider>
  );
}
